...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        # Important to write the log infos for debugging
        logging.info("Entered the data ingestion method or component")
        try:
            df = pd.read_csv('notebook\data\stud.csv')
            logging.info('Read the dataset as dataframe')

            # Create the directory to store the training dataset
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path), exist_ok=True)

            # Create the raw dataset as csv file
            df.to_csv(self.ingestion_config.raw_data_path, index=False, header=True)
            logging.info("Train test split initiated")

            # Split the dataset into train and test datasets with a seed
            train_set, test_set = train_test_split(df, test_size=0.2, random_state=42)

            # Create the directory to store the two splits
